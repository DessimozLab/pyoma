...
if __name__ == "__main__":
    ...
    parser.add_argument("--tree", required=True, help="Path to the species tree in newick format")
    ...
    logging.basicConfig(level=30 - 10 * min(conf.v, 2), format="%(asctime)-15s %(levelname)s %(name)-8s %(message)s")

    res = pyoma.browser.ancestral_synteny.infer_synteny(conf.orthoxml, conf.hdf5, conf.tree)
    try:
