...
class HomoeologExtractor(HomeologsConfidenceCalculator):
    fields_to_load = ['EntryNr1', 'EntryNr2', 'Confidence', 'Distance', 'SyntenyConservationLocal']

    def add_more_protein_features(self, df):
        entry_tab = self.h5_handle.root.Protein.Entries
        entries = pandas.DataFrame(entry_tab.read_where(
            '(EntryNr >= {}) & (EntryNr <= {})'
            .format(self.genome_range[0], self.genome_range[1])))
        entries = entries[['EntryNr', 'Chromosome', 'SubGenome']]
        for c in ('Chromosome', 'SubGenome'):
            entries[c] = entries[c].str.decode("utf-8")
        entries = entries.assign(OmaID=entries['EntryNr'].apply(
            lambda x: "{:s}{:05d}".format(self.genome, x - self.genome_range[0] + 1)))

        new_df = pandas.merge(pandas.merge(df, entries, how='left', left_on="EntryNr1", right_on="EntryNr"),
                              entries, how='left', left_on="EntryNr2", right_on="EntryNr")
        new_df.drop(columns=['EntryNr_x', 'EntryNr_y'], inplace=True, errors="ignore")
        new_df.rename(columns={'OmaID_x': 'OmaID1', 'OmaID_y': 'OmaID2',
                               'Chromosome_x': 'Chromosome1', 'Chromosome_y': 'Chromosome2',
                               'SubGenome_x': 'SubGenome1', 'SubGenome_y': 'SubGenome2'},
                      inplace=True)
        return new_df
...
if __name__ == "__main__":
    ...
    cols = ["OmaID1", "SourceID1", "Chromosome1", "SubGenome1",
            "OmaID2", "SourceID2", "Chromosome2", "SubGenome2",
            "Confidence", "Distance", "SyntenyConservationLocal"]
