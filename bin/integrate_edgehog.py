...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Integrate results from EdgeHOG into oma browser database")
    parser.add_argument("-v", action="count", default=0, help="increase verbosity")
    parser.add_argument("--omadb", required=True, help="path to OmaServer.h5 file")
    parser.add_argument("--edgehog", required=True, help="Path to edgehog result in hdf5 format")

    conf = parser.parse_args()
    log_level = 30 - (10 * min(conf.v, 2))
    logging.basicConfig(level=log_level)
    logger.info("Params: %s", conf)
    with tables.open_file(conf.edgehog, "r") as edge_h5, tables.open_file(conf.omadb, "a") as h5:
