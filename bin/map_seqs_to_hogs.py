...
if __name__ == "__main__":
    ...
    parser.add_argument('fasta', nargs="+", help="File(s) with fasta formatted protein sequences. Checkpointing does only work with a single input file so far.")
        ...
        for infn in conf.fasta:
            with open(infn, 'rt') as fin:
                ...
                try:
                    ...
                    if len(conf.fasta) > 1:
                        logger.warning("checkpointing does only properly work with a single input file. You used more than one file. please investigate how to recover")
                        sys.exit(98)
                    sys.exit(99)
