...
for gtdbid, cand in maxpairs.items():
    if gtdbid in taxid2code:
        ...
        if gtdbid not in pyomatax["NCBITaxonId"]:
            ...
            print(
                f"{gtdbid} - {this_one['gtdb_name']} not in pyomatax -> {tnode.taxid} - {t.translate_to_names([tnode.taxid])[0]}"
            )
        else:
            this_one["valid_oma_tax"] = True
        for k in range(len(cand)):
            if cand[k][1] < 0.4 * cand[0][1]:
                break
        rel_cand = [
            (
                x[0],
                t.translate_to_names([x[0]])[0],
                x[1],
                rapidfuzz.fuzz.ratio(t.translate_to_names([x[0]])[0], this_one["gtdb_name"]),
            )
            for x in cand[:k]
        ]
    ...
    if ncbiid in taxid2code:
        ...
        if 2759 in t.get_lineage(ncbiid) or ncbiid in (2, 2157):
            if ncbiid not in pyomatax["NCBITaxonId"]:
                ...
                this_one["representative"] = tnode.taxid
                print(
                    f"{ncbiid} - {this_one['sciname']} not in pyomatax -> {tnode.taxid} - {t.translate_to_names([tnode.taxid])[0]}"
                )
            else:
                this_one["valid_oma_tax"] = True
            ancestral.append(this_one)
        ...
        rel_cand = [
            (
                ...
                t.translate_to_names([x[0]])[0],
                x[1],
                rapidfuzz.fuzz.ratio(t.translate_to_names([x[0]])[0], this_one["sciname"]),
            )
            for x in cand[:k]
        ]
        rel_cand.sort(key=lambda e: (-e[2], -e[3]))
