...
def chunked(iterator, dtype, n):
    ...
    while True:
        ...
        collections.deque(
            zip(itertools.islice(cnt_it, n), counter), maxlen=0
        )  # (consume at C speed)
...
class GenomeImportanceHelper:
    dtype = numpy.dtype(
        [("EntryOff", "u4"), ("NrExpGO", "i4"), ("NrCurGO", "i4"), ("Priority", "i4")]
    )
    ...
    def sort_numpy_with_entrynr(self, entries: numpy.ndarray):
        ...
        if issubclass(entries.dtype.type, numpy.integer):
            ...
            col = entries["EntryNr"]
        prios = self.genome_data[self.genome_data["EntryOff"].searchsorted(col) - 1][
            "Priority"
        ]
