...
class GOEnrichmentTest(TestWithDbInstance):
    def test_yeast_mapk(self):
        res = pyoma.application.enrichment.extent_species_go_enrichment(
            self.db, [2834, 678, 1003]
        )
    ...
    def test_random_set(self):
        ...
        res = pyoma.application.enrichment.extent_species_go_enrichment(self.db, sample)
