...
class SuffixArraySearchTests(unittest.TestCase):
    ...
    def test_create_index_for_CharCol50(self):
        suf = suffixsearch.SuffixIndexBuilderStringCol(self.h5.get_node('/test/table'),
                                                       'CharCol50', '/test', ignore_case=True)
        suf()
    ...
    def test_create_index_more_than_one_column(self):
        ...
        for col in cols:
            suf = suffixsearch.SuffixIndexBuilderStringCol(self.h5.get_node('/test/table'),
                                                           col, '/test', ignore_case=True)
            suf()
        for col in cols:
            self.assertEqual('/test/{}_buffer'.format(col), self.h5.get_node_attr('/test', col + "_buffer"))
            self.assertEqual('/test/{}_offset'.format(col), self.h5.get_node_attr('/test', col + "_offset"))
    ...
    def test_search_case_insensitive(self):
        ...
        for k, full_query in enumerate(tab.read_coordinates(target_row, field='CharCol50')):
            ...
            self.assertIn(target_row[k], res, "could not find {} (full value {}, row {}) in results ({})"
                          .format(query, full_query, target_row[k], res))
            for target in res:
...
class SuffixArrayVarLenSearchTestsCaseInsensitive(unittest.TestCase):
    ...
    def build_index(self):
        tab = self.h5.get_node('/test/table')
        suffixsearch.SuffixIndexBuilderVarStringCol(tab, 'VarCharOff', self.h5.get_node('/test/buffer'),
                                                    '/test', ignore_case=self.ignore_case)()
    ...
    def test_var_and_fixed_col_have_same_values(self):
        ...
        for row in tab:
            from_buf = buf[row['VarCharOff']:(row['VarCharOff']+row['VarCharLen'])].tostring()
    ...
    def test_existance_of_auxillary_buffers(self):
        ...
        if self.ignore_case:
            self.assertEqual(self.h5.get_node_attr('/test', 'VarCharOff_buffer'), '/test/VarCharOff_buffer')
        self.assertEqual(self.h5.get_node_attr('/test', 'VarCharOff_offset'), '/test/VarCharOff_offset')
    ...
    def test_search_pattern(self):
        ...
        for k, full_query in enumerate(tab.read_coordinates(target_row, field='CharCol')):
            ...
            for target in res:
                if self.ignore_case:
                    self.assertIsNotNone(re.search(query, tab[target]['CharCol'], flags=re.IGNORECASE))
                else:
